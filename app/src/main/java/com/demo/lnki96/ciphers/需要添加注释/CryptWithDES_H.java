...
本类的使用方法:
        ...
        创建一个新的对象,加解密方法需要通过这个对象来调用
        CryptWithDES_H CryptWithDES_H = new CryptWithDES_H(key);

        // 加密后的byte型的密文,加密时DesEncrypt方法的第一个参数为byte[],第二个参数为1
        byte[] result = CryptWithDES_H.DesEncrypt(data.getBytes(), 1);
        System.err.println("加密后密文：" + new String(result));

        // 由于DES为对称加密,所以解密仍然调用DesEncrypt方法,解密时,第一个参数为把byte[]类型的密文,第二个参数为0
        System.err.println("解密后明文：" + new String(CryptWithDES_H.DesEncrypt(result, 0)));

*/
public class CryptWithDES_H {
    byte[] bytekey;

    public CryptWithDES_H(String strKey) {
...
// 特别注意：xxx[IP[i]-1]等类似变换
    ...
    public static void main() {
        ...
        String data = "this is my data to encrypt with des";
        CryptWithDES_H CryptWithDES_H = new CryptWithDES_H(key);
        System.err.println("加密前明文：" + data);
        // 加密后的byte型的密文
        byte[] result = CryptWithDES_H.DesEncrypt(data.getBytes(), 1);
